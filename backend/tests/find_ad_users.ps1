...
# Run this on a domain-joined machine or domain controller
...
if (Get-Module -ListAvailable -Name ActiveDirectory) {
    ...
    try {
        ...
        $users = Get-ADUser -Filter {Enabled -eq $true} -Properties DisplayName,EmailAddress,Description,MemberOf | 
                 Select-Object Name,SamAccountName,EmailAddress,DisplayName,Description
        ...
        if ($users.Count -gt 0) {
            ...
            Write-Host "🧪 TEST THESE ACCOUNTS:" -ForegroundColor Green
            Write-Host "======================" -ForegroundColor Green
            Write-Host "1. Go to: http://localhost/nwrcontractregistry/frontend/index.php" -ForegroundColor White
            Write-Host "2. Try logging in with any of the usernames above" -ForegroundColor White
            Write-Host "3. Use the actual domain password for each user" -ForegroundColor White
            Write-Host ""
...
Write-Host "• Service accounts (if enabled for login)" -ForegroundColor White
Write-Host ""

Write-Host "🎯 TESTING INSTRUCTIONS:" -ForegroundColor Green
Write-Host "========================" -ForegroundColor Green
Write-Host "1. Open the NWR Contract Registry login page" -ForegroundColor White
...
Write-Host "5. Role will be assigned based on AD group membership" -ForegroundColor White
Write-Host ""

Write-Host "🌐 Application URL: http://localhost/nwrcontractregistry/frontend/index.php" -ForegroundColor Cyan